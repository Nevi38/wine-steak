...
namespace wine_steak.Controllers
{
    public class PaymentController : Controller
    {
        ...
        public ActionResult Vnpay(int money)
        {
            string url = ConfigurationManager.AppSettings["vnp_Returnurl"];
            string returnUrl = ConfigurationManager.AppSettings["vnp_Url"];
                ...
                if (checkSignature)
                {
                    if (vnp_ResponseCode == "00")
                    {
                        ...
                        HoaDon hoadon = (from hoaDon in db.HoaDons
                                         where hoaDon.MaSoBan == Int32.Parse(Session["masoban"].ToString()) && hoaDon.isPayment == false
